...
const PleasantsList = () => {
  ...
  const { data: pets, isLoading, isError } = usePets('pleasant');

  const filteredData = pets?.filter((pleasant) =>
    ...
    <div className="bg-[var(--bg-primary)] pb-24">
      ...
      <div className="px-4 md:px-12 max-w-7xl mx-auto mt-16">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-12 gap-6">
          <h1 className="text-4xl md:text-5xl font-black text-[var(--text-primary)] drop-shadow-[3px_3px_0px_var(--accent-teal)]">
            Pretty Pleasants
          </h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
          {isLoading ? (
            ...
            filteredData.map((pleasant) => (
              <Link key={pleasant._id} href={`/pleasant/${pleasant._id}`}>
                <div className="flex flex-col h-full border-4 border-[var(--text-primary)] rounded-[2rem] p-5 bg-white shadow-[8px_8px_0px_var(--text-primary)] hover:-translate-y-2 hover:shadow-[12px_12px_0px_var(--text-primary)] transition-all duration-300 group cursor-pointer">
                  <div className="w-full aspect-[4/3] rounded-2xl overflow-hidden border-4 border-[var(--text-primary)] mb-5 relative bg-[var(--accent-teal)]">
                    <Image
                      src={pleasant.mainImage}
                      fill
                      alt={pleasant.name}
                      className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500"
                    />
                    {/* Status Badge */}
                    <div className={`absolute top-3 left-3 px-4 py-1.5 rounded-full border-4 border-[var(--text-primary)] text-xs font-black uppercase shadow-[4px_4px_0px_var(--text-primary)] ${pleasant.avail?.toLowerCase() === 'available' ? 'bg-[var(--accent-mint)] text-[var(--text-primary)]' : 'bg-[var(--accent-coral)] text-white'}`}>
                      {pleasant.avail}
                    </div>
                    {/* Sale Badge */}
                  ...
                  <h2 className="text-2xl font-black text-[var(--text-primary)] mb-1 line-clamp-1">{pleasant.name}</h2>
                  ...
                  {/* Stock Indicator */}
                  {pleasant.stock > 0 && pleasant.stock <= 5 && (
                    <div className="mb-2 text-xs font-black text-[var(--accent-coral)] animate-pulse">
                      🔥 Only {pleasant.stock} left in stock!
                    </div>
                  )}

                  <p className="text-sm font-medium text-[var(--text-primary)] opacity-80 line-clamp-2 pb-2 flex-grow">{pleasant.description}</p>
                </div>
              </Link>
            ))
          ) : (
