...
const SilkieList = () => {
  ...
  const { data: pets, isLoading, isError } = usePets('silkie');

  const filteredData = pets?.filter((silkie) =>
    silkie.name.toLowerCase().includes(searchTerm.toLowerCase())
  ) || [];

  return (
    <div className="bg-[var(--bg-primary)] pb-24">
      <Landing image='/Silkie/silkie.jpg'/>
      
      <div className="px-4 md:px-12 max-w-7xl mx-auto mt-16">
        {/* Header & Search */}
        <div className="flex flex-col md:flex-row justify-between items-center mb-12 gap-6">
          <h1 className="text-4xl md:text-5xl font-black text-[var(--text-primary)] drop-shadow-[3px_3px_0px_var(--accent-mint)]">
            Fluffy Silkies
          </h1>
          <div className="relative w-full md:w-auto group">
            <input
              ...
              className="w-full md:w-80 border-4 border-[var(--text-primary)] rounded-full px-6 py-4 font-bold text-[var(--text-primary)] outline-none focus:ring-4 focus:ring-[var(--accent-coral)] shadow-[6px_6px_0px_var(--text-primary)] transition-all hover:-translate-y-1 hover:shadow-[8px_8px_0px_var(--text-primary)]"
            />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
          {isLoading ? (
            ...
            filteredData.map((silkie) => (
              <Link key={silkie._id} href={`/silkie/${silkie._id}`}>
                <div className="flex flex-col h-full border-4 border-[var(--text-primary)] rounded-[2rem] p-5 bg-white shadow-[8px_8px_0px_var(--text-primary)] hover:-translate-y-2 hover:shadow-[12px_12px_0px_var(--text-primary)] transition-all duration-300 group cursor-pointer">
                  <div className="w-full aspect-[4/3] rounded-2xl overflow-hidden border-4 border-[var(--text-primary)] mb-5 relative bg-[var(--accent-coral)]">
                    <Image
                      src={silkie.mainImage}
                      fill
                      alt={silkie.name}
                      className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500"
                    />
                    {/* Status Badge */}
                    <div className={`absolute top-3 left-3 px-4 py-1.5 rounded-full border-4 border-[var(--text-primary)] text-xs font-black uppercase shadow-[4px_4px_0px_var(--text-primary)] ${silkie.avail?.toLowerCase() === 'available' ? 'bg-[var(--accent-mint)] text-[var(--text-primary)]' : 'bg-[var(--accent-coral)] text-white'}`}>
                      {silkie.avail}
                    </div>
                    {/* Sale Badge */}
                    {silkie.discountedPrice && (
                  ...
                  <h2 className="text-2xl font-black text-[var(--text-primary)] mb-1 line-clamp-1">{silkie.name}</h2>
                    ...
                    {silkie.discountedPrice ? (
                      <>
                        <span className="text-xl font-black text-[var(--text-primary)] bg-[var(--accent-mint)] px-2 py-0.5 rounded-lg border-2 border-[var(--text-primary)] shadow-[2px_2px_0px_var(--text-primary)]">
                          ₹{silkie.discountedPrice.toLocaleString('en-IN')}
                        </span>
                        <span className="text-sm font-bold text-[var(--text-primary)] opacity-60 line-through decoration-2 decoration-[var(--accent-coral)]">
                          ₹{silkie.basePrice?.toLocaleString('en-IN')}
                        </span>
                      </>
                    ) : (
                      <span className="text-xl font-black text-[var(--text-primary)] drop-shadow-[2px_2px_0px_var(--accent-yellow)]">
                        ₹{silkie.basePrice?.toLocaleString('en-IN') || 'Price Unset'}
                      </span>
                    )}
                  </div>
                  
                  {/* Stock Indicator */}
                  {silkie.stock > 0 && silkie.stock <= 5 && (
                    <div className="mb-2 text-xs font-black text-[var(--accent-coral)] animate-pulse">
                      🔥 Only {silkie.stock} left in stock!
                    </div>
                  )}

                  <p className="text-sm font-medium text-[var(--text-primary)] opacity-80 line-clamp-2 pb-2 flex-grow">{silkie.description}</p>
                </div>
              </Link>
            ))
          ) : (
