        ...
        {filteredData.length > 0 ? (
            filteredData.map((fish) => (
                ...
                <div  className="shadow-lg p-4 bg-white rounded-lg cursor-pointer hover:scale-[1.01] hover:shadow-2xl transition-all duration-1000 ease-in-out">
                    ...
                    <h2 className="text-lg font-bold py-1">{fish.name}</h2>
                    {fish.avail === 'available' ?
                        <p className="text-xs text-green-700 pb-1">{fish.avail}</p>:
                        <p className="text-xs text-red-700 pb-1">{fish.avail}</p>
                    }
