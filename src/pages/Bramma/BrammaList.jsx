...
const BrammaList = () => {
  ...
  const filteredData = data.filter((bramma) =>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-3 p-4">
        {filteredData.length > 0 ? (
            filteredData.map((bramma) => (
                <Link key={bramma.id} href={`/bramma/${bramma.id}`}>
                <div  className="shadow-lg p-4 bg-white rounded-lg cursor-pointer hover:scale-[1.01] hover:shadow-2xl transition-all duration-1000 ease-in-out">
                    ...
                    <h2 className="text-lg font-bold py-1">{bramma.name}</h2>
                    {bramma.avail === 'available' ?
                        <p className="text-xs text-green-700 pb-1">{bramma.avail}</p>:
                        <p className="text-xs text-red-700 pb-1">{bramma.avail}</p>
                    }
                    <p className="text-sm text-gray-600 line-clamp-3">{bramma.description}</p>
                </div>
            </Link>
            ))
        ) : (
