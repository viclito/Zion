...
export default function SilkieDetails({ silkie }) {
  // Initialize the state with the default big image
  const [selectedImage, setSelectedImage] = useState(silkie.mainImage);
      ...
      <div className="h-auto max-w-[1350px] m-auto flex flex-col md:flex-row items-start justify-center md:mt-4">
        <div className="w-[90%] m-auto md:w-[50%] h-[600px] overflow-hidden rounded-lg flex flex-col-reverse md:flex-row justify-start gap-2">
          ...
          <div className="flex md:flex-col flex-row gap-2">
            {[silkie.mainImage, ...(silkie.gallery || []).filter(img => img)].map((item, i) => (
              <div
                key={i}
          ...
          {/* Big Image */}
          <div className="md:w-[70%] w-full rounded-lg overflow-hidden border-4 border-[var(--text-primary)] shadow-[6px_6px_0px_var(--text-primary)] relative bg-[var(--accent-coral)]">
            <Image
        ...
        <div className="w-[90%] md:w-[50%] m-auto my-6 flex flex-col justify-center">
          <h1 className="text-4xl md:text-6xl font-black text-[var(--text-primary)] mb-6 drop-shadow-[3px_3px_0px_var(--accent-coral)]">{silkie.name}</h1>
          ...
          <h3 className="text-xl font-black text-[var(--text-primary)] mb-3 uppercase tracking-wider relative inline-block">
             ...
             <span className="absolute bottom-1 left-0 w-full h-3 bg-[var(--accent-coral)] -z-10 -rotate-1 opacity-70"></span>
          </h3>
          <p className="text-base font-medium text-[var(--text-primary)] opacity-80 mb-10 w-[90%] text-justify leading-relaxed">{silkie.description}</p>

          <OrderSection pet={silkie} />
        </div>
      </div>
      <Footer />
    </>
