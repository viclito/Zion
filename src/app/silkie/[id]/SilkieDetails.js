...
export default function SilkieDetails({ silkie }) {
  ...
  const [selectedImage, setSelectedImage] = useState(silkie.Image);
    ...
    <>
      ...
      <div className="h-auto max-w-[1350px] m-auto flex flex-col md:flex-row items-start justify-center md:mt-4">
        ...
        <div className="w-[90%] md:w-[50%] m-auto my-2">
          <h1 className="text-xl font-semibold mt-4">{silkie.name}</h1>
          <p className="text-sm text-gray-600 mt-2 w-[90%] text-justify">{silkie.description}</p>
          ...
          <ContactForm name={silkie.name} category="Silkie" />
        </div>
      </div>
      <Footer />
    </>
