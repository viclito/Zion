...
export default function PleasantDetails({ pleasant }) {
  ...
  const [selectedImage, setSelectedImage] = useState(pleasant.Image);
    ...
    <>
      ...
      <div className="h-auto max-w-[1350px] m-auto flex flex-col md:flex-row items-start justify-center md:mt-4">
        ...
        <div className="w-[90%] md:w-[50%] m-auto my-2">
          <h1 className="text-xl font-semibold mt-4">{pleasant.name}</h1>
          <p className="text-sm text-gray-600 mt-2 w-[90%] text-justify">{pleasant.description}</p>
          ...
          <ContactForm name={pleasant.name} category="Pleasant" />
        </div>
      </div>
      <Footer />
    </>
